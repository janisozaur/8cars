...
#ifdef TEST_TRAM
    sprintf(outStr, "Tram:\n\rRct2Version=%d &GAMEDATA:base=0x%x varaddr=0x%x", 0, RCT2_ADDR_GAMEDATABASE,
            &RCT2_ADDR_GAMEDATABASE);
    MessageBox(NULL, outStr, "Message", MB_ICONEXCLAMATION | MB_OK | MB_SYSTEMMODAL);
#endif TEST_TRAM

    return WinWrite(mainWindow, RCT2_TITLE, RCT2_ADDR_GUESTGENPROBABILITY, &guestGenProb, sizeof(guestGenProb));
}
