...
int CALLBACK CustMessageDlgProc(HWND   CustMsgDlg, // handle to dialog box
                                UINT   msg,        // message
                                WPARAM wparam,     // first message parameter
                                LPARAM lparam      // second message parameter
                                );

int CALLBACK LandMoverDlgProc(HWND   LandMoverDlg, // handle to dialog box
                              UINT   msg,          // message
                              WPARAM wparam,       // first message parameter
                              LPARAM lparam        // second message parameter
                              );

int CALLBACK GuestEntryDlgProc(HWND   GuestEntryDlg, // handle to dialog box
                               UINT   msg,           // message
                               WPARAM wparam,        // first message parameter
                               LPARAM lparam         // second message parameter
                               );

int CALLBACK RandomTerrainDlgProc(HWND hdlg, UINT msg, WPARAM wparam, LPARAM lparam);

// Set8carsDllLocals(_8CARS_WINDOWTITLE,_Rct2KeyboardHook,_haccel,Rct2Version);
int Set8carsDllLocals(char * newMain, HHOOK newhook, HACCEL newAccel, int _Rct2Version)
{
...
{
    DWORD
    repeatCount : 16, scanCode : 8, isExtendedKey : 1, reserved : 4, altDown : 1, prevKeyState : 1, transitionState : 1;
...
typedef struct
{
    union {
        struct
        {
            BYTE toggled : 1, reserved : 6, down : 1;
        } stateBits;
        BYTE stateByte;
    } shift, ctrl, alt;
} SHIFTS;
...
LRESULT CALLBACK RCT2KeyInterceptProc(int    code,   // hook code
                                      ...
                                      )
{
    ...
    */
    ...
    case VK_F12:
        ...
        {
            if ((!shifts.alt.stateBits.down) && (!shifts.ctrl.stateBits.down) && (!shifts.shift.stateBits.down))
            {
                switch (wparam)
                {
                case VK_F1:
...
int MenuSwitch(HWND hwnd, WORD commandID)
{
    int  result = TRUE, hprocess;
    BYTE byteData;
    char path1[128], path2[128], *cmdline[2] = { &path1, NULL };

    HANDLE
    curProcess = GetCurrentProcess(), curThread = GetCurrentThread();
    ...
    case ID_PARK_ADDCUSTOMMESSAGE:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_INSERTCUSTOMMESSAGE), hwnd, CustMessageDlgProc);
        break;
    ...
    case ID_PARK_CHANGEGUESTENTRYPOINTS:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_GUESTENTRYPOINTS), hwnd, GuestEntryDlgProc);
        break;
    ...
    case ID_LANDSCAPE_GENERATERANDOMTERRAIN:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_RANDOMTERRAIN), hwnd, RandomTerrainDlgProc);
        break;
    ...
    case ID_LANDSCAPE_ADJUSTLANDMOVERSIZE:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_TERRAINTOOLSIZE), hwnd, LandMoverDlgProc);
        break;
    ...
    case ID_STAFF_EDITSTAFF:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_STAFFMENU), hwnd, StaffMenuDlgProc);
        break;

    case ID_RIDES_EDITRIDESTATS:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_RIDESMENU), hwnd, RideMenuDlgProc);
        break;
    ...
    case ID_GUESTS_UNIFORMPEEPS:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_UNIFORMPEEPS), hwnd, UniformPeepColorProc);
        break;
    ...
    case ID_HELP_CONTENTS:
        ...
        hprocess = ShellExecute(hwnd, "open", &path1, NULL, &savepath, 0);
        // hprocess=CreateProcess(&path2,"notepad
        // E:\\8cars\\c\\src\\8carshelp.txt",NULL,NULL,FALSE,0,NULL,NULL,&startup,&processinfo);
    ...
    case ID_MISC_MAPOBJECTMANIPULATION:
        DialogBox(hinst, MAKEINTRESOURCE(IDD_MAPOBJECTMANIPULATION), hwnd, MapObjectManipulationDlgProc);
        break;
    default:
        result = FALSE;
        break;
...
struct DROPDOWNENTRY
{
    ...
    {
      { "white", 0x90 }, { "red", 0x91 },  { "green", 0x92 },  { "gold", 0x93 },  { "brown", 0x94 },
      { "grey", 0x95 },  { "blue", 0x96 }, { "purple", 0x97 }, { "beige", 0x98 }, { "pink", 0x99 },
      { "aqua", 0x9A },  { "tan", 0x9B },  { "green", 0x9C },
    },
  MsgIcon[MESSAGEICONS] = {
      { "Ride", 1 },    { "Person walking", 2 },  { "Person walking", 3 }, { "Coins", 4 }, { "Blank", 5 },
      { "Scenery", 6 }, { "Crowd of people", 7 }, { "Award", 8 },          { "Graph", 9 },
  };
...

{
    ...
    case WM_INITDIALOG:
        ...
        {
            SendMessage(hColor, CB_ADDSTRING, 0, (LPARAM)&MsgColor[i].String);
            SendMessage(hColor, CB_SETITEMDATA, i, (LPARAM)MsgColor[i].ID);
        }
        ...
        {
            SendMessage(hIcon, CB_ADDSTRING, 0, (LPARAM)&MsgIcon[i].String);
            SendMessage(hIcon, CB_SETITEMDATA, i, (LPARAM)MsgIcon[i].ID);
        }
...

{
    ...
    case WM_CHAR:
        ...
        case '+':
        case '=':
            LandMoverDlgProc(LandMoverDlg, WM_COMMAND, IDC_BUTTONPLUS, 0);
            return TRUE;
        case '-':
        case '_':
            LandMoverDlgProc(LandMoverDlg, WM_COMMAND, IDC_BUTTONMINUS, 0);
            return TRUE;
        default:
            return FALSE;
        }
...
struct DROPDOWNENTRY DirectionEntry[4] = { "East", 0, "South", 1, "West", 2, "North", 3 };
                               ...
                               )
{
    ...
    int CoordBoxID[3][2] = { { IDC_XEDIT, IDC_XEDIT2 }, { IDC_YEDIT, IDC_YEDIT2 }, { IDC_ZEDIT, IDC_ZEDIT2 } };
    ...
    case WM_INITDIALOG:
        ...
        for (i = 0; i < 2; i++)
        {
            for (j = 0; j < 4; j++)
            {
                SendMessage(hDirecBox[i], CB_ADDSTRING, 0, (LPARAM)&DirectionEntry[j].String);
                SendMessage(hDirecBox[i], CB_SETITEMDATA, j, DirectionEntry[j].ID);
            }
            ...
            for (j = X; j <= Z; j++)
            {
                ...
                if (RCTLoaded)
                    ...
                    {
                    case X:
                        _itoa(mapEntry[i].x, &tmpstr, 10);
                        break;
                    case Y:
                        _itoa(mapEntry[i].y, &tmpstr, 10);
                        break;
                    case Z:
                        _itoa(mapEntry[i].z, &tmpstr, 10);
                        break;
                    }
                else
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            SetMapEntryPoints(Coord[X][0], Coord[Y][0], Coord[Z][0], Coord[X][1], Coord[Y][1], Coord[Z][1], direction[0],
                              direction[1]);
...
typedef enum { RTC_AVGHEIGHT, RTC_INCLINE, RTC_VARIATION } RT_CONTROL;
...
int CALLBACK RandomTerrainDlgProc(HWND hdlg, UINT msg, WPARAM wparam, LPARAM lparam)
{
    static int barval[HIGHRTCONTROLS], barID[HIGHRTCONTROLS] = { IDC_AVGHEIGHTBAR, IDC_INCLINEBAR, IDC_VARIATIONBAR },
                                       editID[HIGHRTCONTROLS] = { IDC_AVGHEIGHTEDIT, IDC_INCLINEEDIT, IDC_VARIATIONEDIT },

                                       BarRange[HIGHRTCONTROLS][2] = {
                                           { -6, 117 }, { 0, 117 }, { 0, 117 },
                                       };

    static HWND barHndl[HIGHRTCONTROLS] = { NULL, NULL, NULL };
    ...
    case WM_INITDIALOG:
        ...
        { // hedit=GetDlgItem(hdlg,editID[i]);
            ...
            SendMessage(barHndl[i], TBM_SETRANGE, (WPARAM)TRUE, MAKELONG(BarRange[i][LOW], BarRange[i][HIGH]));
...
int APIENTRY AboutBox(HWND hwnd)
{
    ...
    GetFileVersionInfo(_8CARS_EXENAME, 0, dwBytes = GetFileVersionInfoSize(_8CARS_EXENAME, &zero), &Data);

    AboutString[p = 0] = 0;
    p++;
    VerQueryValue(&Data, "\\StringFileInfo\\040904b0\\FileDescription", &s, &dwBytes);
    strcat(AboutString, s);
    strcat(AboutString, "\n\r"
                        "Version ");
    VerQueryValue(&Data, "\\StringFileInfo\\040904b0\\FileVersion", &s, &dwBytes);
    strcat(AboutString, s);
    strcat(AboutString, "\n\r");
    VerQueryValue(&Data, "\\StringFileInfo\\040904b0\\LegalCopyright", &s, &dwBytes);
    strcat(AboutString, s);

    MessageBox(hwnd, &AboutString, "About 8 Cars Per Trainer",
               MB_DEFBUTTON1 | MB_OK | MB_APPLMODAL | MB_SETFOREGROUND | MB_ICONINFORMATION);
}
