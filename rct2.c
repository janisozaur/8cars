...
char RCT2_GAMEDATA_CODESEARCH[] = "\x66\x83\xC1\x16\x66\x83\xC2\x1A\xBE\x1F\x00\x00\x00\x66\xA1";

char RCT2_ADDR_PARKMAPPTRS_CODESEARCH[] = "\x51\x0F\xB7\xF1\xC1\xE6\x08\x66\x0B\xF0\xC1\xEE\x03\x8B\xB6";

char RCT2_ADDR_XYSPRITECHAINS_CODESEARCH[] = "\xC1\xE0\x03\x66\x0B\xC1\x66\x8B\x34\x45";

char RCT2_ADDR_TRACKTYPE_CONSTRUCTOP_CODESEARCH[] = "\x8A\x80\xB5\x01\x00\x00\x0A\xC0\x75\x0A\x0F\xB6\x06\x8A\x04\xC5";
...
char RCT2_ADDR_OBJECTFILEPOINTERS_CODESEARCH[] = "\x72\x02\xFE\xC2\x8A\xF2\x0F\xB6\x5F\x01\x8B\x1C\x9D";

char RCT2_ADDR_LANDMOVERSIZE_CODESEARCH[] = "\x66\x6B\xED\x0F\x66\x6B\xFF\x0F\x66\x03\xC5\x66\x03\xDF\x66\x83\x3D";
...
BYTE RCT2_SHOPTRACKTYPES[RCT2_NUMSHOPTRACKTYPES] = { STT_FOODSTALL, STT_DRINKSTALL, STT_SHOP,    STT_INFOKIOSK,
                                                     STT_RESTROOM,  STT_ATM,        STT_FIRSTAID };
...
short Rct2MapDataFunc(RCT2_MAPDATAFUNC mapdatafunc, short x, short y, LPPARKMAPDATA mapdata, int count)
{
    ...
    {
    case MDF_INIT:
        if (((mapptrs = malloc(sizeof(*mapptrs))) == NULL) || ((mapdatablock = malloc(sizeof(*mapdatablock))) == NULL))
        {
            MessageBox(mainWindow, "Not enough memory (requires appx. 1MB free space)", "Message",
                       MB_DEFBUTTON1 | MB_OK | MB_APPLMODAL | MB_SETFOREGROUND | MB_ICONWARNING);
            return FALSE;
        }
        if ((!WinRead(mainWindow, RCT2_TITLE, mapptrs, RCT2_ADDR_PARKMAPPOINTERS, sizeof(*mapptrs))))
            return FALSE;
        if ((!WinRead(mainWindow, RCT2_TITLE, mapdatablock, RCT2_ADDR_PARKMAP, sizeof(*mapdatablock))))
            return FALSE;
        ...
        else
        {
            ...
            for (y = 0; y < RCT2_HIGHSIZE; y++)
            {
                for (x = 0; x < RCT2_HIGHSIZE; x++)
                {
                    ...
                    memcpy(&(*sortMapDataBlock)[sortI], &(*mapdatablock)[i], tileStructCount * sizeof(PARKMAPDATA));
            ...
            WinWrite(mainWindow, RCT2_TITLE, RCT2_ADDR_PARKMAPPOINTER_NEXTFREE, &nextFreeAddr, sizeof(nextFreeAddr));
            return TRUE;
        }
    }
    return FALSE;
...
#define FOUND_LANDMOVERSIZE (1 << 6)
            ...
            if (memcmp(&RCT2_GAMEDATA_CODESEARCH, &(tmpbuff[i]), sizeof(RCT2_GAMEDATA_CODESEARCH) - 1) == 0)
            {
                if (!(finds & FOUND_PARKDATA))
                {
                    ...
                    RCT2_ADDR_GAMEDATASTARTADDR = addr + sizeof(RCT2_GAMEDATA_CODESEARCH) - 1;
                }
            }
            else if (memcmp(&RCT2_ADDR_PARKMAPPTRS_CODESEARCH, &(tmpbuff[i]), sizeof(RCT2_ADDR_PARKMAPPTRS_CODESEARCH) - 1) ==
                     0)
            {
                if (!(finds & FOUND_PARKMAPPTRS))
                {
                    finds |= FOUND_PARKMAPPTRS;
                    RCT2_ADDR_PARKMAPPTRS_STARTADDR = addr + sizeof(RCT2_ADDR_PARKMAPPTRS_CODESEARCH) - 1;
                }
            }
            else if (memcmp(&RCT2_ADDR_XYSPRITECHAINS_CODESEARCH, &(tmpbuff[i]),
                            sizeof(RCT2_ADDR_XYSPRITECHAINS_CODESEARCH) - 1) == 0)
            {
                if (!(finds & FOUND_XYSPRCHAINDATA))
                {
                    finds |= FOUND_XYSPRCHAINDATA;
                    _RCT2_ADDR_XYSPRITECHAINIS_STARTADDR = addr + sizeof(RCT2_ADDR_XYSPRITECHAINS_CODESEARCH) - 1;
                }
            }
            else if (memcmp(&RCT2_ADDR_TRACKTYPE_CONSTRUCTOP_CODESEARCH, &(tmpbuff[i]),
                            sizeof(RCT2_ADDR_TRACKTYPE_CONSTRUCTOP_CODESEARCH) - 1) == 0)
            {
                if (!(finds & FOUND_TRACKTYPE_CONSTRUCTOP))
                {
                    finds |= FOUND_TRACKTYPE_CONSTRUCTOP;
                    RCT2_ADDR_TRACKTYPE_CONSTRUCTOP_CODEREF = addr + sizeof(RCT2_ADDR_TRACKTYPE_CONSTRUCTOP_CODESEARCH) - 1;
                }
            }
            /*
                #define FOUND_PARKDATA					(1<<0)
                ...
                #define FOUND_LANDMOVERSIZE				(1<<6)
            */
            else if (memcmp(&RCT2_ADDR_RCT2_TRACKPIECE_CONSTRUCTOPPTRS_CODESEARCH, &(tmpbuff[i]),
                            sizeof(RCT2_ADDR_RCT2_TRACKPIECE_CONSTRUCTOPPTRS_CODESEARCH) - 1) == 0)
            {
            ...
            else if (memcmp(&RCT2_ADDR_OBJECTFILEPOINTERS_CODESEARCH, &(tmpbuff[i]),
                            sizeof(RCT2_ADDR_OBJECTFILEPOINTERS_CODESEARCH) - 1) == 0)
            {
                if (!(finds & FOUND_OBJECTFILEPOINTERS))
                {
                    finds |= FOUND_OBJECTFILEPOINTERS;
                    RCT2_ADDR_OBJECTFILEPOINTERS_CODEREF = addr + sizeof(RCT2_ADDR_OBJECTFILEPOINTERS_CODESEARCH) - 1;
                }
            }
            else if (memcmp(&RCT2_ADDR_LANDMOVERSIZE_CODESEARCH, &(tmpbuff[i]),
                            sizeof(RCT2_ADDR_LANDMOVERSIZE_CODESEARCH) - 1) == 0)
            {
                if (!(finds & FOUND_LANDMOVERSIZE))
                {
                    finds |= FOUND_LANDMOVERSIZE;
                    RCT2_ADDR_LANDMOVERSIZE_CODEREF = addr + sizeof(RCT2_ADDR_LANDMOVERSIZE_CODESEARCH) - 1;
                }
            }
            if (finds == (FOUND_PARKDATA | FOUND_PARKMAPPTRS | FOUND_XYSPRCHAINDATA | FOUND_TRACKTYPE_CONSTRUCTOP |
                          FOUND_TRACKPIECE_CONSTRUCTOP | FOUND_OBJECTFILEPOINTERS | FOUND_LANDMOVERSIZE))
                goto EXITLINE;
...
EXITLINE:
    free(tmpbuff);
    // 1
    WinRead(mainWindow, RCT2_TITLE, &RCT2_ADDR_GAMEDATABASE, RCT2_ADDR_GAMEDATASTARTADDR, sizeof(RCT2_ADDR_GAMEDATABASE));
    WinRead(mainWindow, RCT2_TITLE, &_RCT2_ADDR_PARKMAPPOINTERS, RCT2_ADDR_PARKMAPPTRS_STARTADDR,
            sizeof(_RCT2_ADDR_PARKMAPPOINTERS));
    WinRead(mainWindow, RCT2_TITLE, &_RCT2_ADDR_MAPXYSPRITECHAININDEXES, _RCT2_ADDR_XYSPRITECHAINIS_STARTADDR,
    ...
    */
    ...
    WinRead(mainWindow, RCT2_TITLE, &RCT2_ADDR_TRACKTYPECONSTRUCTOPBASE, RCT2_ADDR_TRACKTYPE_CONSTRUCTOP_CODEREF,
            sizeof(RCT2_ADDR_TRACKTYPECONSTRUCTOPBASE));
    WinRead(mainWindow, RCT2_TITLE, &RCT2_ADDRTRACKPIECECONSTRUCTOPBASE, RCT2_ADDR_RCT2_TRACKPIECE_CONSTRUCTOPPTRS_CODEREF,
            sizeof(RCT2_ADDRTRACKPIECECONSTRUCTOPBASE));
    WinRead(mainWindow, RCT2_TITLE, &_RCT2_ADDR_LANDMOVERSIZE, RCT2_ADDR_LANDMOVERSIZE_CODEREF,
            sizeof(_RCT2_ADDR_LANDMOVERSIZE));
    WinRead(mainWindow, RCT2_TITLE, &RCT2_ADDR_OBJECTFILEPOINTERSBASE, RCT2_ADDR_OBJECTFILEPOINTERS_CODEREF,
...
{
    return WinRead(mainWindow, RCT2_TITLE, nameString, ((NAMESTRING *)RCT2_ADDR_NAMESTRINGS)[nameStringIndex],
                   sizeof(NAMESTRING));
}

int SetRCT2NameString(WORD nameStringIndex, BYTE * nameString)
{
    return WinRead(mainWindow, RCT2_TITLE, ((NAMESTRING *)RCT2_ADDR_NAMESTRINGS)[nameStringIndex], nameString,
                   sizeof(NAMESTRING));
}
...
int GetRct2Path(char * path)
{
    ...
    HKEY   hkey;
    long   size = HIGHFULLFILE, datatype = REG_SZ;

    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, RCT2REGKEYNAME, 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
    {
...
int SuspendRCT2()
{
    ...
    rct2Thread    = OpenThreadU(THREAD_SUSPEND_RESUME, FALSE, threadID);
    rct2Suspended = (long)SuspendThread(rct2Thread) == -1;
