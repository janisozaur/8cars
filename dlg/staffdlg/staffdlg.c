...
typedef enum { SMDDL_STAFFSELECT = 0, SMDDL_STAFFTYPESELECT } STAFFMENUDROPDOWNLIST;
...
DROPDOWNBOX SDLG_dropDownList[NUMSTAFFMENUDROPDOWNLISTS] = {
    { { IDC_STAFFSELECT, NULL }, NUMSTAFF, &StaffSelectStrings }, { { IDC_STAFFTYPESELECT, NULL }, NUMSTAFFTYPES, &StaffTypes },
};
...
int StaffChangeProc(HWND hdlg, short index)
{
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &sprite, &(spriteArray[index]), sizeof(sprite)))
        return FALSE;
...
#define GetCBData(i)                                                                                                           \
    SendMessage(SDLG_dropDownList[i].pulldown.hwnd, CB_GETITEMDATA,                                                            \
...
int CALLBACK StaffMenuDlgProc(HWND hdlg, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_INITDIALOG:
        ...
        {
            SDLG_dropDownList[i].pulldown.hwnd = GetDlgItem(hdlg, SDLG_dropDownList[i].pulldown.ID);
            SendMessage(SDLG_dropDownList[i].pulldown.hwnd, CB_RESETCONTENT, 0, 0);

            for (j = 0; j < SDLG_dropDownList[i].numentries; j++)
            {
                SendMessage(SDLG_dropDownList[i].pulldown.hwnd, CB_ADDSTRING, 0, &SDLG_dropDownList[i].entry[j].caption);
                if ((lerror = GetLastError()) != 0)
                    SetLastError(0);

                SendMessage(SDLG_dropDownList[i].pulldown.hwnd, CB_SETITEMDATA, j, SDLG_dropDownList[i].entry[j].data);
                if ((lerror = GetLastError()) != 0)
                    SetLastError(0);

                SDLG_dropDownList[i].entry[j].data = SendMessage(SDLG_dropDownList[i].pulldown.hwnd, CB_GETITEMDATA, j, 0);
            }
            ////
        }
        ...
        if (init)
        {
            ...
            if (!dobreak)
            {
                if (LOWORD(wparam) == SDLG_dropDownList[SMDDL_STAFFSELECT].pulldown.ID)
                {
                    ...
                    {
                        StaffChangeProc(hdlg, SendMessage(SDLG_dropDownList[SMDDL_STAFFSELECT].pulldown.hwnd, CB_GETITEMDATA,
                                                          SendMessage(SDLG_dropDownList[SMDDL_STAFFSELECT].pulldown.hwnd,
                                                                      CB_GETCURSEL, 0, 0),
                                                          0));
                    }
                }
...
int InitStaffList()
{
    ...
    {
        if (!WinRead(mainWindow, RCT2_TITLE, &sprite, &(spriteArray[spriteIndex]), sizeof(sprite)))
            return FALSE;
