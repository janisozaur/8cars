...
landTypeDropListEntries[] = {
    { "Grass", 0 },        { "Sand", 1 },        { "Brown Dirt", 2 }, { "Black Rock", 3 }, { "Redrock", 4 },
    { "Checker", 5 },      { "Grass/Dirt", 6 },  { "Snow", 7 },       { "Red Grid", 8 },   { "Yellow Grid", 9 },
    { "Purple Grid", 10 }, { "Green Grid", 11 }, { "Red Clay", 12 },  { "Tan Dirt", 13 },
};
...
CONTROLBOXDATA objTypeCheckBox[] = {
    { IDC_CHECK_RIDETRACK, NULL },    { IDC_CHECK_SMALLSCENERY, NULL }, { IDC_CHECK_LARGESCENERY, NULL },
    { IDC_CHECK_WALLS, NULL },        { IDC_CHECK_PATHBANNER, NULL },   { IDC_CHECK_PARKENTRANCE, NULL },
    { IDC_CHECK_WATER, NULL },        { IDC_CHECK_LAND, NULL },         { IDC_CHECK_PATH, NULL },
...
DROPDOWNBOX objTypeDropListBox[] = {
    { { IDC__DROP_LIST__NEW_LAND_TYPE, NULL }, NUM_TERRAIN_TEXTURES, &landTypeDropListEntries[0] },
};

#define objTypeDropListBox_Entries (sizeof(objTypeDropListBox) / sizeof(DROPDOWNBOX))
...
CONTROLBOXDATA landTypeCheckBox[] = {
    { IDC_CHECK_GRASS, NULL },   { IDC_CHECK_SAND, NULL },       { IDC_CHECK_BROWNDIRT, NULL },  { IDC_CHECK_ROCK, NULL },
    { IDC_CHECK_REDROCK, NULL }, { IDC_CHECK_CHECKER, NULL },    { IDC_CHECK_GRASSDIRT, NULL },  { IDC_CHECK_SNOW, NULL },
    { IDC_CHECK_REDGRID, NULL }, { IDC_CHECK_YELLOWGRID, NULL }, { IDC_CHECK_PURPLEGRID, NULL }, { IDC_CHECK_GREENGRID, NULL },
    { IDC_CHECK_REDCLAY, NULL }, { IDC_CHECK_TANDIRT, NULL }
};
...
CONTROLBOXDATA edgeTypeCheckBox[] = { { IDC_CHECK_SIDE_DIRT, NULL },
...
CONTROLBOXDATA actionCheckBox[] = {
    { IDC_CHECK_ACTION_RAISELOWER, NULL }, { IDC_CHECK_ACTION_REMOVE, NULL }, { IDC__CHECK__ACTION__CHANGE_LAND_TYPE, NULL },
};
...
PLUSMINUSBOX raiseLowerBox = { { IDC_RAISELOWER_HEIGHTOFFSET_EDIT, NULL },
                               { IDC_RAISELOWER_HEIGHTOFFSET_PLUS, NULL },
                               { IDC_RAISELOWER_HEIGHTOFFSET_MINUS, NULL },
                               0,
                               -127,
                               127 };
...
PLUSMINUSBOX heightRangeBox[] = {
    ...
    { { IDC_HEIGHTRANGE_HIGH_EDIT, NULL },
      { IDC_HEIGHTRANGE_HIGH_PLUS, NULL },
      { IDC_HEIGHTRANGE_HIGH_MINUS, NULL },
      255,
      0,
      255 }
};
...
{
    int  i;
    WORD edgeFlags = 0, groundFlags = 0, objFlags = 0, actionFlags = 0,
...
#define ReadCheck(b, a) (SendMessage(b[a].hwnd, BM_GETCHECK, 0, 0) == BST_CHECKED)
...
#define BitFlag(n) (1 << n)
    ...
    {
        if (MessageBox(hLocDlg, "This will change the height of the specified objects. Press OK to proceed", "Confirmation",
                       MB_APPLMODAL | MB_OKCANCEL | MB_SETFOREGROUND) == IDOK)
    ...
    {
        if (MessageBox(hLocDlg, "This will remove the specified objects. Press OK to proceed", "Confirmation",
                       MB_APPLMODAL | MB_OKCANCEL | MB_SETFOREGROUND) == IDOK)
    ...
    {
        i           = SendMessage(objTypeDropListBox[DRPDN__NEW_LAMD_TYPE].pulldown.hwnd, CB_GETCURSEL, 0, 0);
        newLandType = SendMessage(objTypeDropListBox[DRPDN__NEW_LAMD_TYPE].pulldown.hwnd, CB_GETITEMDATA, i, 0);
        if (MessageBox(hLocDlg, "This will change the land type of specified objects. Press OK to proceed", "Confirmation",
                       MB_APPLMODAL | MB_OKCANCEL | MB_SETFOREGROUND) == IDOK)
...
int CALLBACK MapObjectManipulationDlgProc(HWND hdlg, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_INITDIALOG:
        ...
        {
            objTypeDropListBox[i].pulldown.hwnd = GetDlgItem(hdlg, objTypeDropListBox[i].pulldown.ID);
            objTypeDropListBox[i].entry         = &landTypeDropListEntries;

            for (j = 0; j < NUM_TERRAIN_TEXTURES; j++)
            {
                SendMessage(objTypeDropListBox[i].pulldown.hwnd, CB_ADDSTRING, 0, landTypeDropListEntries[j].caption);
                SendMessage(objTypeDropListBox[i].pulldown.hwnd, CB_SETITEMDATA, j, landTypeDropListEntries[j].data);
            }
        }
