...
CONTROLBOXDATA checkBox[NUMRIDEMENUCHECKBOXES] = { { IDC_FIXBREAKDOWN_CHECK, NULL },  { IDC_RENEW_CHECK, NULL },
                                                   { IDC_MAKERELIABLE_CHECK, NULL },  { IDC_RAISELOWER_CHECK, NULL },
                                                   { IDC_BURY_CHECK, NULL },          { IDC_EXCAVATE_CHECK, NULL },
                                                   { IDC_ZEROCLEARANCE_CHECK, NULL }, { IDC__TOGGLE_VISIBLE__CHECK, NULL },
                                                   { IDC__TOGGLE_CHAIN__CHECK, NULL } };

typedef enum { RMDDL_RIDESELECT = 0, RMDDL_TRACKTYPE, RMDDL_TRAINTYPE, RMDDL_OPERMODE, RMDDL_COLORSCHEME } RIDEMENUDROPDOWNLIST;
...
#define EMPTYSLOTTEXT "<empty>"
    ...
    ColorSchemes[] = { { "Main", COLORCSCHEME_MAIN },
...
DROPDOWNBOX dropDownList[NUMRIDEMENUDROPDOWNLISTS] = { { { IDC_RIDESELECT, NULL }, NUMRIDES, &RideSelectStrings },
...
////
typedef enum { RMC_RIDENAME } RIDEMENUCAPTION;

CONTROLBOXDATA CaptionBox[] = { { IDC_RIDENAME, NULL } };
...
#define STRTABLEBUFFSIZE 512
    ...
    for (i = 0; i < NUMVEHICLES; i++)
    {
        ...
        {
            if (WinRead(mainWindow, RCT2_TITLE, &nameTable, CurRideObjAddr + sizeof(TYPE0HEADER), STRTABLEBUFFSIZE))
            {
...
int RideChangeProc(HWND hdlg, short index)
{
    ...
    short traintype, opmode, tracktype, launchspeed, liftspeed, numtrains, numcars, buildAbsMonth, curAbsMonth, rideage,
        circuits, heightoffset, colorscheme, nameStringIndex,
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &ride, &(ridearray[index]), sizeof(ride)))
        return FALSE;
...
#define SetItemText(a) SetDlgItemText(hdlg, valueBox[a].valueBtn.ID, &tmpstr);
#define DoItoa(a) itoa(a, &tmpstr, 10)
    {
...
#define SetCaption(c) SetDlgItemText(hdlg, CaptionBox[c].ID, &tmpstr)
    {
...
int ApplyChanges(HWND hdlg)
{
    ...
    short index, traintype, opmode, tracktype, launchspeed, liftspeed, numtrains, numcars, buildAbsMonth, curAbsMonth, rideage,
        circuits, heightoffset, colorscheme,
    ...
    short fixbreakdown, raiselower, renew, makereliable, bury, excavate, zeroClearance, toggleVisibility, toggleChain;
...
#define GetCBData(i)                                                                                                           \
    SendMessage(dropDownList[i].pulldown.hwnd, CB_GETITEMDATA, SendMessage(dropDownList[i].pulldown.hwnd, CB_GETCURSEL, 0, 0), \
                0)
    {
...
#define GetItemText(a) GetDlgItemText(hdlg, valueBox[a].valueBtn.ID, &tmpstr, sizeof(tmpstr));
#define DoAtoi atoi(&tmpstr)
    {
...
#define ReadCheck(a) (SendMessage(checkBox[a].hwnd, BM_GETCHECK, 0, 0) == BST_CHECKED)
    {
...
#undef ReadCheck
    ...
    if (raiselower && (heightoffset != 0))
    {
        ...
        case COLORCSCHEME_MAIN:
            RaiseLowerRide(index, heightoffset, colorscheme, &stationRaised);
    ...
    case WM_INITDIALOG:
        ...
        if (!init)
        {
            ...
            for (i = 0; i < NUMRIDES; i++)
            {
                ...
                sprintf(&RideSelectStrings[i].caption, RIDESELECTFORMAT, (RideSelectStrings[i].data = i), tmpStr[0]);
            }
            for (i = 0; i < NUMVEHICLES; i++)
            {
                TrainTypes[i].data = i;
        ...
        {
            dropDownList[i].pulldown.hwnd = GetDlgItem(hdlg, dropDownList[i].pulldown.ID);
            SendMessage(dropDownList[i].pulldown.hwnd, CB_RESETCONTENT, 0, 0);

            for (j = 0; j < dropDownList[i].numentries; j++)
            {
                SendMessage(dropDownList[i].pulldown.hwnd, CB_ADDSTRING, 0, &dropDownList[i].entry[j].caption);
                if ((lerror = GetLastError()) != 0)
                    SetLastError(0);

                SendMessage(dropDownList[i].pulldown.hwnd, CB_SETITEMDATA, j, dropDownList[i].entry[j].data);
                if ((lerror = GetLastError()) != 0)
                    SetLastError(0);

                dropDownList[i].entry[j].data = SendMessage(dropDownList[i].pulldown.hwnd, CB_GETITEMDATA, j, 0);
            }
            ////
        }
            ...
            if (!dobreak)
            {
                if (LOWORD(wparam) == dropDownList[RMDDL_RIDESELECT].pulldown.ID)
                {
                    if (HIWORD(wparam) == CBN_SELCHANGE)
                    {
                        RideChangeProc(
                            hdlg,
                            SendMessage(dropDownList[RMDDL_RIDESELECT].pulldown.hwnd, CB_GETITEMDATA,
                                        SendMessage(dropDownList[RMDDL_RIDESELECT].pulldown.hwnd, CB_GETCURSEL, 0, 0), 0));
                    }
                }
            }
            if (dobreak)
