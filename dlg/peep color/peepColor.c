...
typedef struct
{
    BYTE red, green, blue, index;
} PALETTEENTRY_B;
static PALETTEENTRY_B colorPalette[32] = {
    { 47, 67, 67, 0 },     { 111, 131, 131, 1 }, { 183, 195, 195, 2 }, { 63, 67, 119, 3 },   { 119, 119, 175, 4 },
    { 135, 83, 187, 5 },   { 15, 67, 187, 6 },   { 67, 135, 227, 7 },  { 143, 211, 243, 8 }, { 43, 147, 143, 9 },
    { 131, 207, 207, 10 }, { 31, 123, 0, 11 },   { 79, 135, 95, 12 },  { 99, 143, 59, 13 },  { 71, 175, 39, 14 },
    { 135, 159, 95, 15 },  { 135, 139, 59, 16 }, { 255, 231, 47, 17 }, { 215, 167, 52, 18 }, { 143, 83, 7, 19 },
    { 255, 111, 23, 20 },  { 183, 71, 0, 21 },   { 179, 131, 67, 22 }, { 127, 87, 31, 23 },  { 123, 103, 75, 24 },
    { 207, 131, 99, 25 },  { 163, 59, 59, 26 },  { 143, 0, 0, 27 },    { 227, 7, 0, 28 },    { 143, 23, 79, 29 },
    { 219, 59, 143, 30 },  { 235, 159, 159, 31 }
};

typedef enum { PIU_OVERRIDEPURCHASE = 0 } PEEPSINUNIFORM_CHECKBOX;

#define NUM_PIU_OVERRIDEPURCHASE (PIU_OVERRIDEPURCHASE + 1)

CONTROLBOXDATA checkBoxes[] = { { IDC_OVERRIDEPURCHASECHECK, NULL } };
#define numCheckBoxes (sizeof(checkBoxes) / sizeof(CONTROLBOXDATA))

DROPDOWNBOX cbArray[2] = { { { IDC_SHIRTCOLOR, NULL }, 32, &colorListEntries[0] },
...
int UniPeepColorInitProc(HWND hdlg)
{
    ...
    for (i = 0; i < 32; i++)
    {
        memset(colorListEntries[i].caption, 0, sizeof(colorListEntries[i].caption));
    ...
    for (i = 0; i < cbArray_Entries; i++)
    {
        ...
        cbArray[i].pulldown.hwnd = GetDlgItem(hdlg, cbArray[i].pulldown.ID);
        cbArray[i].entry         = &colorListEntries;

        for (j = 0; j < 32; j++)
        {
            SendMessage(cbArray[i].pulldown.hwnd, CB_ADDSTRING, 0, "");
        }
    }
...
UINT GetPicIndex(hdlg, x, y)
{
    ...
    for (i = 0; i < cbArray_Entries; i++)
    {
        if (inrange(y, startXY[i].y, startXY[i].y + 10) && inrange(x, startXY[i].x, startXY[i].x + 383))
        {
...
int UniPeepColorApplyProc()
{
    ...
    for (i = 0; i < cbArray_Entries; i++)
    {
        if ((*clothesIndex[i] = SendMessage(cbArray[i].pulldown.hwnd, CB_GETCURSEL, 0, 0)) == CB_ERR)
            return FALSE;
    }

#define ReadCheck(a) (SendMessage(checkBoxes[a].hwnd, BM_GETCHECK, 0, 0) == BST_CHECKED)
    {
...
#define GetBackground(state) (state & ODS_SELECTED ? COLOR_HIGHLIGHT : COLOR_WINDOW)
#define GetForeground(state) (state & ODS_SELECTED ? COLOR_HIGHLIGHTTEXT : COLOR_WINDOWTEXT)

int CALLBACK UniformPeepColorProc(HWND hdlg, UINT msg, WPARAM wparam, LPARAM lparam)
{
    int                 x, y, index;
    ...
    case WM_DRAWITEM:
        ...
        clrForeground = SetTextColor(lpdis->hDC, GetForeground(lpdis->itemState));
        clrBackground = SetBkColor(lpdis->hDC, GetBackground(lpdis->itemState));
        ...
        drawBrush = CreateSolidBrush(
            RGB(colorPalette[lpdis->itemID].red, colorPalette[lpdis->itemID].green, colorPalette[lpdis->itemID].blue, ));
        ...
        if ((lpdis->itemState & (ODS_COMBOBOXEDIT | ODS_FOCUS)) && (lpdis->hwndItem == GetFocus()))
        {
