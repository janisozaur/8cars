...
int SetMapHeight(int * init, int done, int x, int y, PARKMAPPTRARRAY * lpMapPtrs, LPSCANLINE scanline, WORD parksize)
{
...
#define SE (int)((*scanline)[y + 1][x + 1] & 0XFE)
    ...
    int nw_corner, ne_corner, sw_corner, se_corner, CornertoCorner2, NWSE_Test, NESW_Test;
    ...
    // if(inWriteRange=PointInRect(&ValidXYRange, MakePoint(x,y)))
    {
        ...
        n_sign = range(N - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (n_sign < 0)
            n_sign += RCT2_HEIGHTUNIT;
        s_sign = range(S - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (s_sign < 0)
            s_sign += RCT2_HEIGHTUNIT;
        e_sign = range(E - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (e_sign < 0)
            e_sign += RCT2_HEIGHTUNIT;
        w_sign = range(W - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (w_sign < 0)
            w_sign += RCT2_HEIGHTUNIT;
        ne_sign = range(NE - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (ne_sign < 0)
            ne_sign += RCT2_HEIGHTUNIT;
        nw_sign = range(NW - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (nw_sign < 0)
            nw_sign += RCT2_HEIGHTUNIT;
        se_sign = range(SE - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (se_sign < 0)
            se_sign += RCT2_HEIGHTUNIT;
        sw_sign = range(SW - C, -(RCT2_HEIGHTUNIT << 1), RCT2_HEIGHTUNIT);
        if (sw_sign < 0)
            sw_sign += RCT2_HEIGHTUNIT;
        ...
        CornertoCorner2 = (abs(nw_corner - se_corner) == 2) ^ (abs(ne_corner - sw_corner) == 2);
        ...
        Slope = ((nw_corner & 1) << NW_BIT) | ((ne_corner & 1) << NE_BIT) | ((sw_corner & 1) << SW_BIT) |
                ((se_corner & 1) << SE_BIT) | ((CornertoCorner2 & 1) << 4);
    ...
    if (*init)
    {
        ...
        do
        {
            ...
            moretiles = ((!(pmd[i].byte1.lastElementOnTile)) && ((i + 1) < 512));
            i++;
...
////
int GetMapHeight(int * init, int done, int x, int y, PARKMAPPTRARRAY * lpMapPtrs, LPSCANLINE scanline, WORD parksize)
{
...
int SaveTerrainBMP(HWND hwnd, HINSTANCE hinstance)
{
    ...
    */
    ...
    BMPFILEHEAD_256
    bmpFilehead = { {
                        BM_ID, 0, 0, 0, sizeof(BMPFILEHEAD_256),
                    },
                    {
                        { /*
                             DWORD  biSize;
                          ...
                          */
                          0, 0, 0, 1, 8, BI_RGB, 0, 720, 720, 256, 256 }
    ...
    ofn.Flags             = OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_LONGNAMES | OFN_OVERWRITEPROMPT;
    // ofn.nFileOffset;
    // ofn.nFileExtension;
    ofn.lpstrDefExt = "BMP";
    ;
    ...
    if (WinRead(mainWindow, RCT2_TITLE, &parksize, RCT2_ADDR_MAPSIZE, 2))
    {
        ...
        bmpFilehead.bmfh.bfSize =
            sizeof(BMPFILEHEAD_256) + (bmpFilehead.bmih.bmiHeader.biSizeImage = EvenUp(parksize) * parksize);
...
#define palcolor(i, c) (bmpFilehead.bmih.bmiColors[i].rgb##c)
        {
            for (i = 0; i < 256; i++)
            {
                palcolor(i, Red) = palcolor(i, Green) = palcolor(i, Blue) = i;
...
#undef palcolor
        ...
        if (parksize >= 3)
        {
            ...
            for (y = 1; y <= parksize - 2; y++)
            {
                ...
                {
                    GetMapHeight(&init, FALSE, x, y, &MapPtrs, &scanline, parksize);
                    if (!init)
                        goto ExitFor;
                }
            }
        ...
        ExitFor:
            ...
            if (init)
            {
                if ((hfile = _sopen(&filetitle, _O_BINARY | _O_CREAT | _O_SEQUENTIAL | _O_RDWR, _SH_DENYNO,
                                    _S_IREAD | _S_IWRITE)) == -1)
                    goto ExitIf;

                _write(hfile, &bmpFilehead, sizeof(BMPFILEHEAD_256));

                svNBytes = bmpFilehead.bmih.bmiHeader.biSizeImage / bmpFilehead.bmih.bmiHeader.biHeight;
...
int LoadTerrainBMP(HWND hwnd, HINSTANCE hinstance)
{
    ...
    hfile = CreateFile(&filetitle, GENERIC_READ, 0, &secuAttrb, OPEN_EXISTING,
    ...
    if (WinRead(mainWindow, RCT2_TITLE, &parksize, RCT2_ADDR_MAPSIZE, 2))
    {
        ...
        if ((bmpFilehead.bmfh.bfType == BM_ID) && (bmpFilehead.bmih.biBitCount == 8) &&
            (bmpFilehead.bmih.biWidth == bmpFilehead.bmih.biHeight) && (bmpFilehead.bmih.biWidth == parksize) &&
            (bmpFilehead.bmih.biCompression == BI_RGB) && (parksize >= 3))

        {
            ...
            for (y = 1; y <= parksize - 2; y++)
            {
                for (x = 1; x <= parksize - 2; x++)
                {
                    SetMapHeight(&init, FALSE, x, y, &MapPtrs, &scanline, parksize);
                    if (!init)
                        goto ExitFor;
                }
            }
