...
int WriteRct2Message(char * msgString, short color, short icon, short month, short day)
{
    MESSAGESTRUCT message;

    if (FindWindow(NULL, RCT2_TITLE) == NULL)
        return FALSE;
    ...
    WinWrite(mainWindow, RCT2_TITLE, RCT2_ADDR_MESSAGES, &message, sizeof(message));
}
...
int ClearAllMessages()
{
    ...
    {
        WinWrite(mainWindow, RCT2_TITLE, messageptr, &message, sizeof(message));
        ++messageptr;
    }
}
