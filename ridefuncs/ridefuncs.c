...
int RideFunc(BYTE lowI, BYTE highI, RIDEFUNCID func)
{
    ...
    {
        WinRead(mainWindow, RCT2_TITLE, &ride, &((*ridePtrBase)[i]), sizeof(ride));
...
#define MAXNUMSTATIONS 4
        ...
        WinWrite(mainWindow, RCT2_TITLE, &((*ridePtrBase)[i]), &ride, sizeof(ride));
    }
...
int EditTrackSpecs(TRACKSPECFUNC tsfunc)
{
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &trackSpecs, specAddr, sizeof(trackSpecs)))
        return FALSE;
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, rideObjPtrs, RCT2_ADDR_RIDEOBJECTFILEPOINTERS, sizeof(rideObjPtrs)))
        return FALSE;
    ...
    for (i = 0; i < RCT2_OBJMAX_RIDES; i++)
    {
        ...
        {
            WinRead(mainWindow, RCT2_TITLE, &curRideoObj, rideObjPtrs[i], sizeof(curRideoObj));
            ...
            case TSF_8CARSPERTRAIN:
                curRideoObj.maxCarsPerTrain = max(curRideoObj.maxCarsPerTrain, 15);
                goto CASE_TSFCHANGECARSPERTRAIN;
            CASE_TSFCHANGECARSPERTRAIN:
                curRideoObj.minCarsPerTrain = min(curRideoObj.minCarsPerTrain, 1);
                break;
            ...
            WinWrite(mainWindow, RCT2_TITLE, rideObjPtrs[i], &curRideoObj, sizeof(curRideoObj));
        }
    }

    WinWrite(mainWindow, RCT2_TITLE, specAddr, &trackSpecs, sizeof(trackSpecs));
