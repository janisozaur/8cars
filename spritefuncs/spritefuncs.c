...
SortSpriteChain(SPRITESTRUCTARRAY * spriteBlock, SPRITE_MAPXYINDEXES * mapXYChainIndexes, short SpriteID,
                short * firstSpriteIndex)
{
...
SortHandymanTiles(SPRITESTRUCTARRAY * spriteBlock, SPRITE_MAPXYINDEXES * mapXYChainIndexes, short SpriteID,
                  short * firstSpriteIndex)
{
...
EmptyRidesOfGuests(SPRITESTATS * spritestats, SPRITESTRUCT * spriteBlock)
{
    WORD       index;
    ...
    while (index != RCT2_WORD_EMPTY)
    {
        WinRead(mainWindow, RCT2_TITLE, &vehiOwner, &(rideArray[spriteBlock[index].byte30.ownerRide]), sizeof(vehiOwner));
        if (vehiOwner.trackType != 0x43) // mini golf
        {
            memset(&(spriteBlock[index].union52.guestsInVehicle), RCT2_BYTE_EMPTY,
                   sizeof(spriteBlock[index].union52.guestsInVehicle)); // empty the seats in vehicles
            spriteBlock[index].unionB0.vehi.passengerCount[0] = spriteBlock[index].unionB0.vehi.passengerCount[1] = 0;
        }
    ...
    while (index != RCT2_WORD_EMPTY)
    {
        ...
        {
            memset(&(spriteBlock[index].union52.guestsInVehicle), RCT2_BYTE_EMPTY,
                   sizeof(spriteBlock[index].union52.guestsInVehicle));
            spriteBlock[index].unionB0.vehi.passengerCount[0] = spriteBlock[index].unionB0.vehi.passengerCount[1] = 0;
        }
        index = spriteBlock[index].nextInClass;
    }
...
WORD GetPrevOnTile(WORD x, WORD y, WORD index, SPRITESTRUCTARRAY * sprites, SPRITE_MAPXYINDEXES * mapXYChainIndexes)
{
    WORD retV = RCT2_WORD_EMPTY, curIndex = (*mapXYChainIndexes)[x][y];
    if (index == curIndex)
        retV = RCT2_WORD_EMPTY;
    else
    {
        while (((*sprites)[curIndex].nextOnThisTile != index) && ((*sprites)[curIndex].nextOnThisTile != RCT2_WORD_EMPTY) &&
               (curIndex < NUMSPRITES) && (curIndex != (*sprites)[curIndex].nextOnThisTile))
        {
...
int RemoveSprite(WORD index, WORD prevI, WORD nextI, WORD * prevFreeI, SPRITETYPE spritetype, SPRITESTATS * spritestats,
                 SPRITESTRUCT * cursprite, SPRITESTRUCTARRAY * sprites, SPRITE_MAPXYINDEXES * mapXYChainIndexes)
{
...
#define cursprite ((*spriteBlock)[index])
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &spritestats, RCT2_ADDR_SPRITESTATS, sizeof(SPRITESTATS)))
        return FALSE;
    ...
    case SF_FLOAT:
        ...
        if ((spriteBlock = malloc(sizeof(SPRITESTRUCTARRAY))) == NULL)
        {
            MessageBox(mainWindow, "Not enough memory (requires appx. 2.5 MB free space)", "Message",
        ...
        {
            MessageBox(mainWindow, "Not enough memory (requires appx. 32 KB free space)", "Message",
                       MB_DEFBUTTON1 | MB_OK | MB_APPLMODAL | MB_SETFOREGROUND | MB_ICONWARNING);
            return FALSE;
        }
#ifdef RCT2_FREEZE
        ...
        WinRead(mainWindow, RCT2_TITLE, spriteBlock, spritePtrBase, sizeof(*spriteBlock));
        WinRead(mainWindow, RCT2_TITLE, mapXYChainIndexes, mapXYChainIndexesPtrBase, sizeof(*mapXYChainIndexes));
        ...
        case SF_GUEST:
            ...
            if ((index = spritestats.statsA.firstPerson) == RCT2_WORD_EMPTY || (spritestats.statsA.peopleCount == 0))
                break;
            ...
            do
            {
                ...
                if (cursprite.persontype == SPR_PT_GUEST)
                {
                    ...
                    case GUEST_PUTINUNIFORM:
                        if (PPIU_overridePurchases || (!(cursprite.items & ITEM_TSHIRT)))
                        {
                    ...
                    case GUEST_REMOVEALL:
                        ...
                        case 17: // at _____*,
                            // RIDESTRUCT rideOn,*rideArray=RCT2_ADDR_RIDES;
                            WinRead(mainWindow, RCT2_TITLE, &rideOn,
                                    &(rideArray[cursprite.union52.personStruct.unkCurRideOnI_A]), sizeof(rideOn));
                    ...
                    RemoveGuestLine:
                        if ((cursprite.guestAdmissionStatus & GAS_OUTOFPARK) == GAS_INPARK)
                            --newAttendance;
                        RemoveSprite(index, prevI, nextI, &prevFreeI, SPR_PERSON, &spritestats, &cursprite, spriteBlock,
                                     mapXYChainIndexes);
        ...
        case SF_TRASH:
            ...
            if (((index = spritestats.statsA.firstJunk) == RCT2_WORD_EMPTY) || (spritestats.statsA.junkCount == 0))
                break;
            ...
            do
            {
                ...
                {
                case TRASH_REMOVEALL:
                    RemoveSprite(index, prevI, nextI, &prevFreeI, SPR_JUNK, &spritestats, &cursprite, spriteBlock,
                                 mapXYChainIndexes);
                    break;
                }
        ...
        case SF_FLOAT:
            if (((index = spritestats.statsA.firstFloat) == RCT2_WORD_EMPTY) || (spritestats.statsA.floatingCount == 0))
                break;
            ...
            do
            {
                ...
                {
                case FLOAT_REMOVEALL:
                    RemoveSprite(index, prevI, nextI, &prevFreeI, SPR_FLOAT, &spritestats, &cursprite, spriteBlock,
                                 mapXYChainIndexes);
                    break;
                }
        ...
        WinWrite(mainWindow, RCT2_TITLE, RCT2_ADDR_MAPXYSPRITECHAININDEXES, mapXYChainIndexes, sizeof(*mapXYChainIndexes));
        WinWrite(mainWindow, RCT2_TITLE, RCT2_ADDR_SPRITES, spriteBlock, sizeof(*spriteBlock));

        WinWrite(mainWindow, RCT2_TITLE, RCT2_ADDR_SPRITESTATS, &spritestats, sizeof(SPRITESTATS));
...
int PutPeepsInUniform(WORD shirtColor, WORD pantsColor, WORD overridePurchases)
{
...
int AllAroundGuests()
{
    ...
    if (!VitalizeGuests())
        retval = FALSE;
    if (!HelpGuestsFindRestrooms())
        retval = FALSE;
    if (!GiveGuestDrinks())
        retval = FALSE;
    if (!GiveGuestsBurgers())
        retval = FALSE;
    if (!PacifyVandals())
        retval = FALSE;
    //	if(!())retval=FALSE;
...
int ResetGuestNumbers()
{
    ...
    WinWrite(mainWindow, RCT2_TITLE, ptr, &newNextGuestNumber, sizeof(newNextGuestNumber));
}
