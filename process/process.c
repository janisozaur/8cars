...
int WinReadWrite(HWND callWindow, int Func, LPCSTR WindowName, LPVOID lpDest, LPVOID lpSrc, LPDWORD nBytes)
{
    int    rwResult;
    HANDLE hndl;
    HWND   hwnd;
    ...
    {
        MessageBox(callWindow, "No window", "Message",
    ...
    {
        MessageBox(callWindow, "No process", "Message",
                   MB_DEFBUTTON1 | MB_OK | MB_APPLMODAL | MB_SETFOREGROUND | MB_ICONWARNING);
...
int WinWriteBytes(HWND callWindow, LPCSTR WindowName, LPVOID lpDest, LPVOID lpSrc, LPDWORD nBytes)
{
    return WinReadWrite(callWindow, WR_WRITE, WindowName, lpDest, lpSrc, nBytes);
}

int WinReadBytes(HWND callWindow, LPCSTR WindowName, LPVOID lpDest, LPVOID lpSrc, LPDWORD nBytes)
{
    return WinReadWrite(callWindow, WR_READ, WindowName, lpDest, lpSrc, nBytes);
}

int WinWrite(HWND callWindow, LPCSTR WindowName, LPVOID lpDest, LPVOID lpSrc, DWORD nBytes)
{
    return WinReadWrite(callWindow, WR_WRITE, WindowName, lpDest, lpSrc, &nBytes);
}

int WinRead(HWND callWindow, LPCSTR WindowName, LPVOID lpDest, LPVOID lpSrc, DWORD nBytes)
{
    return WinReadWrite(callWindow, WR_READ, WindowName, lpDest, lpSrc, &nBytes);
}

int WinBitFunc(HWND callWindow, int Func, LPCSTR WindowName, LPVOID lpDest, int mask)
{
    char tmp;
