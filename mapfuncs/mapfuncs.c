...
typedef struct
{
    int    index, colorscheme;
    short *low, *high;
} RIDELOWHIGH;
typedef struct
{
    short index, heightoffset, colorscheme, *stationRaised;
} RIDE_RAISELOWERSTRUCT;
typedef struct
{
    short index, colorscheme;
} RIDE_BURYSTRUCT;
...
int GetFirstIndexOfElementType(RCT2_MAPELEMENTTYPE targetElmtType, PARKMAPDATA * MapData)
{
...
int GetCountOfElementType(RCT2_MAPELEMENTTYPE targetElmtType, PARKMAPDATA * MapData)
{
    int i = 0, elmttype, moretiles = TRUE;
    int c = 0;
...
{
    WORD edgeType;
    return edgeType = ((BYTE)MapData[landI].byte0.elmtSpec06 << LANDTYPEEXTENDER_BITPLACE) |
                      ((MapData[landI].byte4.landslope & EDGEGROUNDTYPE_BITMASK) >> EDGEGROUNDTYPE_BITPLACE);
}

WORD GetLandGroundType(PARKMAPDATA * MapData, int landI)
{
    WORD groundType;
    return groundType = ((BYTE)MapData[landI].byte0.elmtSpec00 << LANDTYPEEXTENDER_BITPLACE) |
                        ((MapData[landI].byte5.terrainWater & EDGEGROUNDTYPE_BITMASK) >> EDGEGROUNDTYPE_BITPLACE);
}
...
SetLandGroundType(PARKMAPDATA * mapData, WORD groundType)
{
    ...
    mapData[0].byte0.elmtSpec00 &= (BYTE)(~GROUNDTYPE_EXTENDERBITMASK);

    mapData[0].byte0.elmtSpec00 |= (BYTE)((gt.hi & 1) << GROUNDTYPE_EXTENDERBITPLACE);

    mapData[0].byte5.terrainWater &= (BYTE)(~EDGEGROUNDTYPE_BITMASK);

    mapData[0].byte5.terrainWater |= (BYTE)((gt.lo & 7) << EDGEGROUNDTYPE_BITPLACE);
}

int GetOwnership(BYTE * ownership, BYTE * border, LPPARKMAPDATA MapData)
{
    *ownership = ((*MapData).byte7.ownership & OWNERSHIP_MASK) >> OWNERSHIP_SHIFT;
    *border    = ((*MapData).byte7.ownership & BORDERFENCE_MASK) >> BORDERFENCE_SHIFT;
}
...
int SetOwnership(LPPARKMAPDATA MapData, int ownership, int border)
{
    ...
    (*MapData).byte7.ownership =
        ((ownership << OWNERSHIP_SHIFT) & OWNERSHIP_MASK) | ((border << BORDERFENCE_SHIFT) & BORDERFENCE_MASK);

    return (*MapData).byte7.ownership;
}

// memswap(&MapData[LandI],&MapData[i],sizeof(PARKMAPDATA));
int SwapMapDataElements(PARKMAPDATA * MapData, int i1, int i2)
{
    int t2LastTile;

    if (i1 > i2)
        memswap(&i2, &i1, sizeof(i1));
...
{
    WORD count = GetMapTileElementCount(MapData), i, j, landI, landCount, landEdgeType, landGroundType;
    BYTE landBaseHeight;
        ...
        for (i = 0; i < (count - 1); i++)
        {
            for (j = i + 1; j < count; j++)
            {
                ...
                case SORT_BYBASEHEIGHT:
                    if (MapData[j].baseheight < MapData[i].baseheight)
            ...
            case MF_ALL:
                ...
                if (fclass == MF_ALL)
                {
                    if (func == ALL__CHANGE_LAND_TYPE)
                        allLandMatch = (allChangeLand.edgeFlags & (1 << landEdgeType)) ||
                                       (allChangeLand.groundFlags & (1 << landGroundType));
                    else
                        allLandMatch = (allRaiseLower.edgeFlags & (1 << landEdgeType)) ||
                    ...
                    //	if(PointInRect(&ValidXYRange, MakePoint(x,y)))
                    {
                        ...
                        case MF_CLEARANCE:
                            ...
                            case CLEARANCE_RESTOREALL:
                                MapData[i].clearheight = MapData[i].baseheight + GetElementClearance(FALSE, &(MapData[i]));
                                break;
                            }
                            break;
                                ...
                                case MF_RIDE:
                                    ...
                                    case ALL_RAISELOWER:
                                        if (allLandMatch && (allRaiseLower.objFlags & OTF_RIDETRACK) &&
                                            NumInRange(MapData[i].baseheight, allRaiseLower.lowHeight,
                                                       allRaiseLower.highHeight))
                                    ...
                                    case RIDE_GETLOWHIGH:
                                        ...
                                        {
                                            *ridelowhigh.low  = min(*ridelowhigh.low, (BYTE)MapData[i].baseheight);
                                            *ridelowhigh.high = max(*ridelowhigh.high, (BYTE)MapData[i].baseheight);
                                        }
                                        break;
                                    ...
                                    case RIDE_BURY:
                                        if ((bury.colorscheme == (MapData[i].byte6.color & COLORCSCHEME_MASK)) &&
                                            (MapData[i].byte7.rideindex == (BYTE)(bury.index)))
                                        {
                                        ////
                                        LINE_RIDE_BURY:
                                            if ((LandI != RCT2_WORD_EMPTY) &&
                                                (MapData[LandI].baseheight <= MapData[i].clearheight))
                                            {
                                                MapData[LandI].baseheight = MapData[LandI].clearheight =
                                        ...
                                        break;
                                    case RIDE_EXCAVATE:
                                        if ((excavate.colorscheme == (MapData[i].byte6.color & COLORCSCHEME_MASK)) &&
                                    ...
                                    case RIDE_ZEROCLEARANCE:
                                        if ((zeroclearance.colorscheme == (MapData[i].byte6.color & COLORCSCHEME_MASK)) &&
                                    ...
                                    case RIDE__TOGGLE_VISIBILITY:
                                        if ((toggleVisibility.colorscheme == (MapData[i].byte6.color & COLORCSCHEME_MASK)) &&
                                            (MapData[i].byte7.rideindex == (BYTE)(toggleVisibility.index)))
                                        {
                                        LINE__RIDE__TOGGLE_VISIBILITY:
                                            if (i > 1)
                                            {
                                                MapData[i - 1].byte0.data = (BYTE)0XFF;
                                                moretiles                 = false;
                                            }
                                        }
                                        break;
                                    ...
                                    case RIDE__TOGGLE_CHAIN:
                                        if ((toggleChain.colorscheme == (MapData[i].byte6.color & COLORCSCHEME_MASK)) &&
                            ...
                            case MET_SURFACE:
                                ...
                                case MF_OWN:
                                    ...
                                    case OWN_UNOWNTORIGHTS:
                                        if (ownership == 0)
                                            ownership = (OWN_RIGHTSOWNED | OWN_RIGHTSAVAIL);

                                        break;

                                    case OWN_DEOWNBORDER:
                                        if ((x == 1) || (y == 1) || (x == (parksize - 2)) || (y == (parksize - 2)))
                                            ownership = OWN_UNOWNED;
                                        else
                                            goto skipwrite_001;
                                        break;
                                ...
                                case MF_LAND:
                                    ...
                                    {
                                    case LAND_MOWGRASS:
                                        MapData[i].byte6.grass = (MapData[i].byte6.grass & ~GRASSSTATE_MASK) | GRASSSTATE_MOWED;
                                        break;

                                    case LAND_MOWGRASSPLAIN:
                                        MapData[i].byte6.grass = (MapData[i].byte6.grass & ~GRASSSTATE_MASK) | GRASSSTATE_CLEAR;

                                        break;

                                    case LAND_CAPATMAXHEIGHT:
                                        MapData[i].baseheight = MapData[i].clearheight =
...
////
int RaiseLowerRide(short index, short heightoffset, short colorscheme, short * stationRaised)
{
...
int RaiseLowerObjects(WORD objFlags, WORD groundFlags, WORD edgeFlags, WORD lowHeight, WORD highHeight, short offset)
{
    ALL_RAISELOWERSTRUCT allRaiseLower = { objFlags, groundFlags, edgeFlags, lowHeight, highHeight, offset };

    return MapFunc(MF_ALL, ALL_RAISELOWER, &allRaiseLower);
}
int RemoveObjects(WORD objFlags, WORD groundFlags, WORD edgeFlags, WORD lowHeight, WORD highHeight)
{
    ALL_REMOVESTRUCT allRemove = { objFlags, groundFlags, edgeFlags, lowHeight, highHeight };

    return MapFunc(MF_ALL, ALL_REMOVE, &allRemove);
}

int ChangeLandType(WORD newLandType, WORD groundFlags, WORD edgeFlags)
{
    ALL__CHAMGE_LAND_STRUCT allChangeLand = { newLandType, groundFlags, edgeFlags };
...
int SetMapEntryPoints(int x1, int y1, int z1, int x2, int y2, int z2, short dir1, short dir2)
{
    short mapsize = 0;
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &mapsize, RCT2_ADDR_MAPSIZE, 2))
        return FALSE;
    ...
    WinWrite(mainWindow, RCT2_TITLE, RCT2_ADDR_MAPENTRY, &mapEntry, sizeof(mapEntry));
}
/*
