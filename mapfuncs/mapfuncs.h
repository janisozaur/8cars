...
typedef enum {
    ...
    OWN_DEOWNBORDER,
    OWN_OWNALLLAND,
    OWN_RESTOREBORDERFENCE,
    OWN_REMOVEBORDERFENCE,
    LAND_MOWGRASS,
    LAND_MOWGRASSPLAIN,
    LAND_CAPATMAXHEIGHT,
    LAND_FIXINVISIBLEBORDER,
    LAND_DELETEEXTRAS,
    SCEN_WATERGARDENS,
    PATH_EMPTYTRASH,
    PATH_FIXVANDALISM,
    PATH_VANDALIZE,
    CLEARANCE_ZEROALL_ABSOLUTELY,
    CLEARANCE_ZEROALL_RELATIVELY,
    CLEARANCE_RESTOREALL,
    RIDE_GETLOWHIGH,
    RIDE_RAISELOWER,
    RIDE_BURY,
    RIDE_EXCAVATE,
    RIDE_ZEROCLEARANCE,
    RIDE__TOGGLE_VISIBILITY,
    RIDE__TOGGLE_CHAIN,
    SORT_BYBASEHEIGHT,
    ALL_RAISELOWER,
    ALL_REMOVE,
    ALL_GETCOUNT,
    ALL__CHANGE_LAND_TYPE
} MAPFUNCID;

typedef enum { MF_CLEARANCE, MF_OWN, MF_LAND, MF_SCEN, MF_PATH, MF_RIDE, MF_SORT, MF_ALL } MAPFUNCCLASS;
...
typedef enum { SETF_DIRT = 1 << 0, SETF_BROWNWOOD = 1 << 1, SETF_BLACKWOOD = 1 << 2, SETF_ICE = 1 << 3 } SURFACEEDGETYPEFFLAG;
...
{
    WORD fill0;
    BYTE clearance, fill3, cursorType, scenType;
    WORD unkIndex;
