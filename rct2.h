...
typedef enum { RCT2_VER_US = 0, RCT2_VER_DEMO, RCT2_VER_EURO } RCT2_VERSION;
...
#define RCT2_ADDR_PARKMAPPOINTER_NEXTFREE ((void *)((DWORD)(_RCT2_ADDR_PARKMAPPOINTERS) + RCT2_HIGHSIZE * RCT2_HIGHSIZE * 4))
/*Park Map Pointers*/
...
#define RCT2_ADDR_OBJECTFILEPOINTERS RCT2_ADDR_OBJECTFILEPOINTERSBASE
#define RCT2_ADDR_RIDEOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0X09ACFA4 - 0x09ACFA4))
#define RCT2_ADDR_SMALLSCENERYOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09AD1A4 - 0X09ACFA4))
#define RCT2_ADDR_LARGESCENERYOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09AD594 - 0X09ACFA4))
#define RCT2_ADDR_WALLOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09AD794 - 0X09ACFA4))
#define RCT2_ADDR_PATHBANNEROBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09AD994 - 0X09ACFA4))
#define RCT2_ADDR_PATHOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09ADA14 - 0X09ACFA4))
#define RCT2_ADDR_PATHADDITIONOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09ADA54 - 0X09ACFA4))
#define RCT2_ADDR_SCENERYGROUPOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09ADA90 - 0X09ACFA4))
#define RCT2_ADDR_PARKENTRANCEOBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09ADADC - 0X09ACFA4))
#define RCT2_ADDR_WATEROBJECTFILEPOINTERS (RCT2_ADDR_OBJECTFILEPOINTERSBASE + (0x09ADAE0 - 0X09ACFA4))
/*Object Pointers*/

/*Track Type Cconstruction Parameters*/
BYTE * RCT2_ADDR_TRACKTYPECONSTRUCTOPBASE;

#define RCT2_ADDR_TRACKTYPECONSTRUCTOP (RCT2_ADDR_TRACKTYPECONSTRUCTOPBASE + (0x097CF40 - 0x097CF40))
...
{
    short WeatherTimer;
    BYTE  CurrentWeather, TargetWeather, CurrentTemp, TargetTemperature, Lightning, TargetLightning, Shade, TargetShade, Rain,
        TargetRain;
} WEATHER;
...
{
    BYTE  pieceI;
    short xOfs, yOfs, zOfs;
    BYTE  clearance, unknown0, flags;
} TRACKPIECESPECSTRUCT, TRACKPIECESPECSTRUCTARRAY[255];
////
typedef struct
{
    BYTE MaxHeight, // From ground
        ClearanceBase;
    BYTE CarDisplacment;
    BYTE MaxTrainLength, fill4[4];
...
typedef struct
{
    union {
        struct
        {
            unsigned char elmtSpec00 : 2, elmtType : 3, unk05 : 1, elmtSpec06 : 2;
        };
        BYTE data;
    };
} MAPBYTE0;
...
typedef union {
    BYTE elmtID, landslope, bannerindex;
} MAPBYTE4;
...
typedef union {
    BYTE index, scenAge, terrainWater, pathAdditions, parkEntryIndex, bannerPosition;
} MAPBYTE5;
...
typedef union {
    BYTE ownership, rideindex, pathAdditionStatus, item;
} MAPBYTE7;
...
typedef enum { MDF_READ, MDF_WRITE, MDF_INIT, MDF_DONE, MDF_OPTIMIZE } RCT2_MAPDATAFUNC;
...
typedef union {
    struct
    {
        unsigned short nextFreeSprite, firstVehicle, firstPerson, firstFloat, firstJunk, firstOversizedRideCar, availSprites,
            vehiCount, peopleCount, floatingCount, junkCount, oversizedCount;
    } statsA;
    unsigned short statsB[2][SPRITETYPES];
} SPRITESTATS;

typedef enum { SPR_FREE = 0, SPR_VEHI, SPR_PERSON, SPR_FLOAT, SPR_JUNK, SPR_OVERSIZED } SPRITETYPE;
...
typedef struct
{
    BYTE trackType, vehiObjID, fill002[2], opmode, colorScheme;
    struct
    {
        BYTE primary, trim;
    } vehicolors1[32];
    BYTE  fill046[3], ridestatus;
    short nameStringIndex;
    long  stringNumerator;
    RCTPOINT
    overallViewFocus, stationFoci[4];
    BYTE stationHeights[4], stationLengths[4], stationSignal[4], trainsInLoadingPos[4];
    RCTPOINT
    rideEntrances[4], rideExits[4];
    short lastQueueGuestIndex[4];
    BYTE  fill082[4];
    short TrainSpriteIndexes[32];
    BYTE  DepartureControlFlags, amtStations, amtTrains, amtCarsPerTrain, reqAmtTrains, reqAmtCarsPerTrain, maxTrains,
        fill0CD[1], minWait, maxWait, launchSpeed, fill0D1[3], dataLoggingIndex, specialTrackElements, fill0D6[2];
    short maxspeedLo, maxspeedHi, avgspeedLo, avgspeedHi;
    BYTE  fill0E0[4];
    long  rideLengths[4];
    short rideTimes[4], maxPosVertGs, maxNegVertGs, maxLatGs;
    BYTE  fill102[0x12], inversions, drops, fill116[1], highDrop, fill118[8];
    short curCustomerCount, customerCountTimer, lastCustomerCounts[10], ticketPrice;
    BYTE  fill13A[6];
    short excitement, intensity, nausea;
    BYTE  fill146[5], satisfaction, fill14C[4];
    long  totalCustomers, totalProfit;
    BYTE
        /*
        ...
        */
        ...
        15C		1	Music being played:
                    ...
                    45	Candy
        */
        currentMusicPlaying, fill15D[0X180 - 0x15D];
    short absBuiltMonth, runningCost;
    BYTE  fill184[0xF], breakdownType, fill194[2], reliabilityCountdown, reliability, fill198[1], downtime, inspectionFrequency,
        timeSinceLastInspection, fill19C[0x1A8 - 0X19C];
    short onRidePhotosSold;
    BYTE  fill1AA[0X1B8 - 0X1AA], queuetime, fill1B9[0X1CD - 0X1B9], liftChainSpeed;
    short favRideGuestCount, rideStatusFlags;
    BYTE  fill1D2[2], seatColors[32], fill1F4[3], numCircuits, fill1F8[8], guestsInQueue[4], fill204[0X260 - 0X204];
