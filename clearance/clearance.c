...
{
    BYTE StrTbl[255 * LNG_PORTUGESE], leadCh, ch;
    int  pos;
...
{
    WORD rideI = (BYTE)MapData->byte7.rideindex, elmtID = (BYTE)MapData->byte4.elmtID, pieceI = (BYTE)MapData->byte5.index,
         clearance, trackType, vehiObjID;
    BYTE *ptr, bytevalue, i, elmtArrayIndex;
    ...
    *pieceSpecsPtr;
    ...
    WinRead(mainWindow, RCT2_TITLE, &ride, ptr = (RIDESTRUCT *)RCT2_ADDR_RIDES + rideI, sizeof(ride));
    trackType = ride.trackType & 0xff;
    vehiObjID = ride.vehiObjID & 0xff;

    ptr = RCT2_ADDR_TRACKTYPECONSTRUCTOP;

    if (!WinRead(mainWindow, RCT2_TITLE, &trackSpecs, ptr = &(((TRACKSPECSTRUCT *)RCT2_ADDR_TRACKTYPECONSTRUCTOP)[trackType]),
    ...
    WinRead(mainWindow, RCT2_TITLE, &rideObjPtr, (DWORD *)ptr += vehiObjID, sizeof(rideObjPtr));
    WinRead(mainWindow, RCT2_TITLE, &rideObjHeader, rideObjPtr, sizeof(rideObjHeader));
    ...
    WinRead(mainWindow, RCT2_TITLE, &pieceSpecsPtr, (DWORD *)ptr += elmtID, sizeof(pieceSpecsPtr));
...
{
    WORD elmtID = (BYTE)MapData->byte4.elmtID;
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &smallScenObjHeaderPtr, (DWORD *)ptr += elmtID, sizeof(smallScenObjHeaderPtr)))
        return 0;
...
int GetLargeSceneryClearance(LPPARKMAPDATA MapData)
{
    ...
    WORD elmtID = (BYTE)MapData->byte4.elmtID, pieceI = (BYTE)MapData->byte5.index >> 2, retv;
    ...
    *ptrarray = 0;
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &largeScenObjHeaderPtr, &(ptrarray[elmtID]), sizeof(largeScenObjHeaderPtr)))
        return 0;
...
{
    WORD entrytype = (BYTE)MapData->byte4.elmtID;

    switch (entrytype)
    {
    case EE_RIDEEXIT:
        return EE_RIDEEXIT_CLEARANCE;
    case EE_RIDEENTRY:
        return EE_RIDEENTRY_CLEARANCE;
    case EE_PARKGATE:
        return EE_PARKGATE_CLEARANCE;
    default:
        return 0;
    }
}
...
int GetFenceWallSceneryClearance(LPPARKMAPDATA MapData)
{
    ...
    *ptrarray;
    ...
    if (!WinRead(mainWindow, RCT2_TITLE, &fenceWallScenObjHeaderPtr, ptrarray = &(ptrarray[elmtID]),
    ...
    {
        WinRead(mainWindow, RCT2_TITLE, &fenceWallScenObjHeader, fenceWallScenObjHeaderPtr, sizeof(fenceWallScenObjHeader));
...
short GetElementClearance(int init, LPPARKMAPDATA MapData)
{
    ...
    static BYTE WallClearances[RCT2_OBJMAX_WALLS], SScenClearances[RCT2_OBJMAX_SMALLSCENERY];
